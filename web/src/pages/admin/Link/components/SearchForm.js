...
class SearchForm extends Component {
    ...
    return (
      <Form onSubmit={this.handleSearchSubmit} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={8} sm={24}>
            <FormItem label="搜索">
              {getFieldDecorator('search')(<Input placeholder="名称 / 网址"/>)}
            </FormItem>
          </Col>
  ...
  handleSearchSubmit = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      if (err) return;
      dispatch({ type: 'adminLink/setState', payload: { searchData: fieldsValue } });
      dispatch({ type: 'adminLink/get', payload: fieldsValue });
    });
  };
  ...
  handleSearchFormReset = () => {
    ...
    form.resetFields();
    dispatch({ type: 'adminLink/setState', payload: { searchData: {} } });
